...
import { ArrowRight, CheckCircle2, Clock, Laptop, Shield, Star, Truck, Zap, Gift, Percent, Calendar, Award, Tag, ArrowLeft } from 'lucide-react';
...
export default function HomePage() {
  ...
  const laptops = [
    {
      ...
      name: "HP Pavilion",
      price: "₹45,999",
      image: "https://i.rtings.com/assets/pages/6S2WXjTl/best-laptops-20240516-medium.jpg?format=auto",
    ...
    },
    {
      ...
      price: "₹65,999",
      image: "https://datascripmall.id/media/Laptop_Dell.jpg",
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentLaptopIndex((prev) => (prev + 1) % laptops.length);
    }, 3000);
  ...
  return (
    <div className="relative">
      ...
      {/* New Laptop Showcase Section */}
      <section className="relative py-32 overflow-hidden bg-gradient-to-b from-[#0D0D0D] via-[#1A1A1A] to-[#3A1C71]/40" style={{ zIndex: 1 }}>
        {/* Animated Border */}
        <div className="absolute inset-0 z-0">
          <div className="absolute inset-0 bg-gradient-to-r from-[#3A1C71]/20 via-transparent to-[#3A1C71]/20 animate-gradient-x"></div>
          <div className="absolute inset-0 bg-gradient-to-b from-[#3A1C71]/20 via-transparent to-[#3A1C71]/20 animate-gradient-y"></div>
        </div>
        ...
        <div className="container mx-auto px-4 relative z-10 max-w-7xl">
          ...
          >
            <h2 className="text-5xl font-bold mb-6 text-white">Featured Laptops</h2>
            <p className="text-xl text-neutral-300">Discover our premium collection of laptops</p>
          </motion.div>
          ...
          <div className="relative">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                transition={{ duration: 0.5 }}
                className="bg-[#1A1A1A]/50 backdrop-blur-sm rounded-3xl p-12 shadow-2xl border border-[#FFD700]/20 hover:border-[#FFD700]/40 transition-colors"
              >
                <div className="flex flex-col md:flex-row items-center gap-12">
                  ...
                  <div className="w-full md:w-1/2">
                    <motion.div
                      ...
                      className="relative"
                    >
                      <div className="absolute inset-0 bg-gradient-to-t from-[#0D0D0D]/90 via-transparent to-transparent rounded-2xl"></div>
                      <img
                        src={laptops[currentLaptopIndex].image}
                        alt={laptops[currentLaptopIndex].name}
                  ...
                  <div className="w-full md:w-1/2 text-center md:text-left">
                    <h3 className="text-3xl font-bold mb-6 text-white">{laptops[currentLaptopIndex].name}</h3>
                    <p className="text-4xl font-bold text-[#FFD700] mb-6">{laptops[currentLaptopIndex].price}</p>
                    <p className="text-xl text-neutral-300 mb-8">{laptops[currentLaptopIndex].specs}</p>
                    <button
                      onClick={() => navigate('/laptops')}
                      className="px-6 py-2 bg-[#3A1C71] text-white rounded-lg hover:bg-[#4A2C81] transition-colors duration-300"
                    >
                      Learn More
                    </button>
                  </div>
            ...
            {/* Navigation Buttons */}
            <button
              onClick={() => setCurrentLaptopIndex((prev) => (prev - 1 + laptops.length) % laptops.length)}
              className="absolute left-4 top-1/2 -translate-y-1/2 bg-[#1A1A1A]/80 p-3 rounded-full hover:bg-[#2A2A2A] transition-colors border border-[#FFD700]/20 hover:border-[#FFD700]/40"
            >
              <ArrowLeft className="w-8 h-8 text-white" />
            </button>
            <button
              onClick={() => setCurrentLaptopIndex((prev) => (prev + 1) % laptops.length)}
              className="absolute right-4 top-1/2 -translate-y-1/2 bg-[#1A1A1A]/80 p-3 rounded-full hover:bg-[#2A2A2A] transition-colors border border-[#FFD700]/20 hover:border-[#FFD700]/40"
            >
